...
class LiveDataApp(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        frame = OptionsPane(self)
        frame.config(height='800', width='300',bg='powder blue')
        frame.grid_propagate(0)
        self.frames[OptionsPane] = frame
        frame.grid(row=0, column=0, padx=10, pady=10)

        frame2 = PlotPane(self)
        frame2.config(height='800', width='750', bg='linen')
        frame2.grid_propagate(0)
        self.frames[PlotPane] = frame2
        ...
        self.status_bar = tk.Label(self, textvariable=self.var_status_msg, relief=tk.SUNKEN, anchor=tk.W, font=('Calibri', 12))
    ...
    def track_trip(self, options):
        ...
        opt_pane.display_msg('Added New Trip with ID: ' + str(options['trip_id']))
    ...
    def remove_local_record(self, trip_id):
        if trip_id in trip_ids:
            ...
            record = next((rec for rec in db if rec['trip_id'] == trip_id), None)
            if record:
                db.remove(record)
            self.ids_local_update()
            self.db_local_update()
            options_pane = self.get_pane(OptionsPane)
            options_pane.display_msg('Removed Trip record with ID: ' + str(trip_id))
        return
...
class OptionsPane(tk.Frame):
    def __init__(self, controller):
        ...
        self.lbl_main = tk.Label(self, text='Track Trip', font=('Arial', 14), bg='powder blue')
        self.lbl_main.grid(row=0, columnspan=2, pady=2)

        self.var_airline = tk.StringVar(self)
        self.var_airline.set('Choose Airline')
        self.menu_airline = tk.OptionMenu(self, self.var_airline, 'Choose Airline', 'Easyjet', 'Ryanair')
        self.menu_airline.grid(row=1, columnspan=2)

        self.one_way = tk.IntVar()
        self.check_way = tk.Checkbutton(self, text='One Way', variable=self.one_way, onvalue=1, offvalue=0, height=2, width=8, bg='powder blue', command=self.one_way_check)
        self.check_way.grid(row=2, columnspan=2)

        # Source City
        self.lbl_src_city = tk.Label(self, text='Source City', bg='powder blue')
        self.lbl_src_city.grid(row=3, column=0, pady=2, sticky='e')

        self.var_src_city = tk.StringVar(self)
        self.var_src_city.set('Edinburgh')
        self.menu_src_city = tk.OptionMenu(self, self.var_src_city, 'Krakow', 'Edinburgh')
        self.menu_src_city.config(width=10)
        self.menu_src_city.grid(row=3, column=1, pady=2)

        # Destination city
        self.lbl_dst_city = tk.Label(self, text='Destination City', bg='powder blue')
        self.lbl_dst_city.grid(row=4, column=0, pady=2, sticky='e')

        self.var_dst_city = tk.StringVar(self)
        self.var_dst_city.set('Krakow')
        self.menu_dst_city = tk.OptionMenu(self, self.var_dst_city, 'Krakow', 'Edinburgh')
        ...
        self.lbl_back_date = tk.Label(self, text='Return Date', bg='powder blue')
        ...
        self.btn_add_adult = tk.Button(self, text='Adult', command=self.add_adult)
        self.btn_add_adult.grid(row=7, column=0, padx=5, pady=5, sticky='e')
        self.num_adults = tk.IntVar()
        self.num_adults.set(1)
        self.lbl_num_adults = tk.Label(self, textvariable=self.num_adults, font=('Arial', 14), bg='powder blue')
        self.lbl_num_adults.grid(row=7, column=1, sticky='news', padx=5, pady=5)

        self.btn_add_child = tk.Button(self, text='Child', command=self.add_child)
        self.btn_add_child.grid(row=8, column=0, padx=5, pady=5, sticky='e')
        self.num_child = tk.IntVar()
        self.num_child.set(0)
        self.lbl_num_child = tk.Label(self, textvariable=self.num_child, font=('Arial', 14), bg='powder blue')
        self.lbl_num_child.grid(row=8, column=1, sticky='news', padx=5, pady=5)

        self.btn_add_trip = tk.Button(self, text='Add Trip', command=self.add_trip)
        self.btn_add_trip.grid(row=9, columnspan=2, padx=5, pady=5)

        self.lbl_separator = tk.Label(self, text='---------------------------------------------------------', bg='powder blue')
        self.lbl_separator.grid(row=10, columnspan=3)

        self.lbl_db_controls = tk.Label(self, text='Trip Settings', font=('Arial', 14), bg='powder blue')
        self.lbl_db_controls.grid(row=11, columnspan=2)

        self.btn_update_trips = tk.Button(self, text='Sync Data', width=10, command=self.controller.update_trips)
        self.btn_update_trips.grid(row=12, column=0, padx=5, pady=5)

        self.btn_remove_trip = tk.Button(self, text='Remove Trip', width=15, command=self.controller.remove_trip)
        ...
        self.menu_curr_ids = tk.OptionMenu(self, self.var_trip_id, *self.ids_lst, command=self.show_trip_details)
        self.menu_curr_ids.config(width=10)
        self.menu_curr_ids.grid(row=13, column=1, pady=5)

        self.lbl_info = tk.Label(self, text='Info', font=('Calibri', 12), bg='powder blue')
        self.lbl_info.grid(row=14, columnspan=2, padx=5, pady=(20, 1))
        self.out_info = tk.Text(self, state='disabled', width=25, height=15, wrap=tk.WORD, bg='gainsboro', borderwidth=2, relief='groove')
    ...
    def update_ids_menu(self):
        ...
        for id in self.ids_lst:
            menu.add_command(label=id, command=lambda value=id: self.var_trip_id.set(value))
    ...
    def animate(self, i):
        ...
        for idx, trip in enumerate(db):
            ...
            if trip['one_way']:
                ...
                if ys:
                    if ys[-1] == 0:
                        lbl = f"to {trip['dst_city']} Price: SOLD OUT"
                    elif len(ys) > 1:
                        if ys[-1] > ys[-2]:
                            lbl = f"to {trip['dst_city']} Price: {ys[-1]} +{(ys[-1]-ys[-2])/ys[-2]*100:.1f}%"
                        elif ys[-1] < ys[-2]:
                            lbl = f"to {trip['dst_city']} Price: {ys[-1]} -{(ys[-2] - ys[-1])/ys[-2] * 100:.1f}%"
                        else:
                            lbl = f"to {trip['dst_city']} Price: {ys[-1]} +0%"
                    else:
                        lbl = f"to {trip['dst_city']} Price: {ys[-1]} +0%"
                else:
                    lbl = f"to {trip['dst_city']} curr. price: None"
            ...
            if not trip['one_way']:
                ...
                if ys:
                    if ys[-1] == 0:
                        lbl1 = f"to {trip['dst_city']} Price: SOLD OUT"

                    elif len(ys) > 1:
                        if ys[-1] > ys[-2]:
                            lbl1 = f"to {trip['dst_city']} Price: {ys[-1]} +{(ys[-1]-ys[-2])/ys[-2]*100:.1f}%"
                        elif ys[-1] < ys[-2]:
                            lbl1 = f"to {trip['dst_city']} Price: {ys[-1]} -{(ys[-2] - ys[-1])/ys[-2] * 100:.1f}%"
                        else:
                            lbl1 = f"to {trip['dst_city']} Price: {ys[-1]} +0%"

                    else:
                        lbl1 = f"to {trip['dst_city']} Price: {ys[-1]} +0%"

                else:
                    lbl1 = f"to {trip['dst_city']} Price: None"

                if ys2:
                    if ys2[-1] == 0:
                        lbl2 = f"to {trip['src_city']} Price: SOLD OUT"
                    elif len(ys2) > 1:
                        if ys2[-1] > ys2[-2]:
                            lbl2 = f"to {trip['src_city']} Price: {ys2[-1]} +{(ys2[-1]-ys2[-2])/ys2[-2]*100:.1f}%"
                        elif ys2[-1] < ys2[-2]:
                            lbl2 = f"to {trip['src_city']} Price: {ys2[-1]} -{(ys2[-2]-ys2[-1])/ys2[-2] * 100:.1f}%"
                        else:
                            lbl2 = f"to {trip['src_city']} Price: {ys2[-1]} +0%"
                    else:
                        lbl2 = f"to {trip['src_city']} Price: {ys2[-1]} +0%"
                else:
                    lbl2 = f"to {trip['src_city']} Price: None"
